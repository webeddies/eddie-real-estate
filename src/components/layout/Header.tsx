      ...
      <nav className={`transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`text-gray-700 hover:text-royal-blue font-medium transition-colors relative group ${
                  location.pathname === item.path ? 'text-royal-blue' : ''
                }`}
              >
                {item.name}
          ...
          <div className="hidden lg:block">
            <button className="bg-royal-blue text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center space-x-2">
              <Calendar size={18} />
            ...
            <div className="px-4 py-4 space-y-4">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`block text-gray-700 hover:text-royal-blue font-medium transition-colors ${
                    location.pathname === item.path ? 'text-royal-blue' : ''
                  }`}
              ...
              <button className="w-full bg-royal-blue text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 mt-4">
                <Calendar size={18} />
                <span>Schedule a Visit</span>
              </button>
            </div>
